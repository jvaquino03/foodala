// Local mock catalog — the website runs entirely on this in mock-data mode
// (no Supabase), mirroring apps/customer-mobile/src/data/mockRestaurants.ts so
// both apps show the same restaurants, menus, and photography. Shapes mirror the
// database types; the extra presentation fields live on a MockRestaurant superset.

import type { MenuCategory, MenuItem, Restaurant } from '@/types/database';

// Real, dish-accurate food photography. Stable CDN URLs from TheMealDB (meals)
// and TheCocktailDB (drinks). Keyed by restaurant / category / item id.
...
export function getRestaurants(): MockRestaurant[] {
  return MOCK_RESTAURANTS.filter((r) => r.is_active).sort((a, b) =>
    a.name.localeCompare(b.name)
  );
}

/** Featured restaurants for the home hero. */
export function getFeaturedRestaurants(): MockRestaurant[] {
  return MOCK_RESTAURANTS.filter((r) => r.is_active && r.featured);
}

/** A single restaurant by id, or null if not found. */
export function getRestaurantById(id: string): MockRestaurant | null {
  return MOCK_RESTAURANTS.find((r) => r.id === id) ?? null;
}

/** Categories (sorted) with their items attached, for a restaurant. */
...
/** Look up a restaurant name by id (used by the admin orders table). */
export function restaurantName(id: string): string {
  return MOCK_RESTAURANTS.find((r) => r.id === id)?.name ?? 'Unknown';
}
